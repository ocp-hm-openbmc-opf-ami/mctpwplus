...
{
    bool cbCalled = false;
    mctpw::spawn(*io, [this, &cbCalled](boost::asio::yield_context yield) {
        {
        ...
        {
            auto status = mctpw::methodCall(
                *conn, service, objectPath, interface, "TriggerDeviceDiscovery",
                std::make_optional<boost::asio::yield_context>(yield));
            EXPECT_FALSE(status);
        }
        cbCalled = true;
